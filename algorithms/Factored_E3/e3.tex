...
\note{The algorithm is categorized as indirect or model based, which means that the algorithm maintains a partial model of the underlying Markov Decision Process (MDP). However, the agent does not necessarily build a complete model of the underlying MDP if it is not necessary to achieve a high return.} \todo{ta bort/flytta?}
