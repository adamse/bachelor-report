...
\item[Policy evaluation] \hfill \\
  ...
  the update rule \eqref{eq:vfupdate} until $V$ converges. $\pi(s, a)$ is the probability of
  action $a$ in state $s$ using policy $\pi$.
...
\begin{equation} \label{eq:polimpr}
  \pi_{k+1} (s) = \operatorname*{arg\,max}_a \sum_{s'}P(s, a, s') \left[ R(s, a) + \gamma V_k(s') \right]
\end{equation}
